...
/**
 * EJB 2.x Stateless Session Bean — Ordine.
...
public class OrdineBean implements SessionBean {
    ...
    private static Ordine mapOrdine(ResultSet rs) throws SQLException {
        Ordine o = new Ordine();
        o.setId(rs.getInt("id"));
        o.setNumeroOrdine(rs.getString("numero_ordine"));
        o.setClienteId(rs.getInt("cliente_id"));
        o.setIndirizzoId(rs.getInt("indirizzo_id"));
        o.setStato(rs.getString("stato"));
        o.setTotale(rs.getBigDecimal("totale"));
        o.setNote(rs.getString("note"));
        o.setEmailInviata(rs.getBoolean("email_inviata"));
    ...
    public Ordine getOrdineByNumero(String numeroOrdine) {
        ...
        try {
            ...
            ps = con.prepareStatement(
                "SELECT * FROM mercato.ordine WHERE numero_ordine = ?");
            ps.setString(1, numeroOrdine);
            rs = ps.executeQuery();
            return rs.next() ? mapOrdine(rs) : null;
        } catch (Exception e) { throw new EJBException("getOrdineByNumero failed", e);
    ...
    public List<Ordine> getOrdiniCliente(int clienteId) {
        ...
        try {
            con = getConnection();
            ps = con.prepareStatement(
                "SELECT * FROM mercato.ordine WHERE cliente_id = ? ORDER BY created_at DESC");
            ps.setInt(1, clienteId);
            rs = ps.executeQuery();
            List<Ordine> list = new ArrayList<Ordine>();
            while (rs.next()) list.add(mapOrdine(rs));
            return list;
        } catch (Exception e) { throw new EJBException("getOrdiniCliente failed", e);
        } finally { closeQuietly(rs, ps, con); }
    }
    ...
    public void aggiornaStato(int ordineId, String nuovoStato) {
        ...
        try {
            con = getConnection();
            ps = con.prepareStatement(
                "UPDATE mercato.ordine SET stato = ? WHERE id = ?");
            ps.setString(1, nuovoStato);
            ps.setInt(2, ordineId);
            int rows = ps.executeUpdate();
            if (rows == 0) throw new EJBException("aggiornaStato: ordine not found " + ordineId);
        } catch (EJBException ee) { throw ee;
        } catch (Exception e) { throw new EJBException("aggiornaStato failed", e);
        } finally { closeQuietly(null, ps, con); }
    }
