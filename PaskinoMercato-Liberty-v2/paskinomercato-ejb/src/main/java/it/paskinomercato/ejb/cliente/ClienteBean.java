...
/**
 * EJB 2.x Stateless Session Bean — Cliente.
...
public class ClienteBean implements SessionBean {
    ...
    public Cliente registra(String email, String passwordHash, String nome,
        ...
        try {
            ...
            if (!rs.next()) throw new EJBException("INSERT cliente returned no key");
    ...
    public Cliente getClienteById(int id) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            return rs.next() ? mapCliente(rs) : null;
        } catch (Exception e) { throw new EJBException("getClienteById failed", e);
    ...
    public Cliente getClienteByEmail(String email) {
        ...
        try {
            ...
            rs = ps.executeQuery();
            return rs.next() ? mapCliente(rs) : null;
        } catch (Exception e) { throw new EJBException("getClienteByEmail failed", e);
        } finally { closeQuietly(rs, ps, con); }
    }
    ...
    public void aggiornaLingua(int clienteId, String lingua) {
        ...
        try {
            ...
            if (rows == 0) throw new EJBException("Cliente not found: " + clienteId);
        } catch (EJBException ee) { throw ee;
        } catch (Exception e) { throw new EJBException("aggiornaLingua failed", e);
        } finally { closeQuietly(null, ps, con); }
    }

    // ----------------------------------------------------------------
    // Address methods — JDBC (no JPA entity for indirizzo)
    // ----------------------------------------------------------------
