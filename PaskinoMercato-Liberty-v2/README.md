...
**Supermercato Online Italiano — Java EE 7 / EJB 3.2 / Open Liberty & WebSphere Liberty**
...
9. [Web Service JAX-WS / SOAP](#web-service-jax-ws-soap)
...
15. [Build e Gestione con Open Liberty](#build-e-gestione-con-open-liberty)
16. [Configurazione del Database](#configurazione-del-database)
17. [Appendice: Configurazione WebSphere Legacy](#appendice-configurazione-websphere-legacy)
18. [Appendice: Deploy su WebSphere Legacy (wsadmin)](#appendice-deploy-su-websphere-legacy-wsadmin)
...
PaskinoMercato è un'applicazione enterprise modernizzata per **Java EE 7** distribuita come EAR su **Open Liberty** o **WebSphere Liberty**. Offre:
...
| Livello | Tecnologia |
|---|---|
| Application Server | **Open Liberty** (o WebSphere Liberty) |
| Versione Java EE | **Java EE 7** (web-app 3.1, EJB 3.2) |
| Bytecode Java | **Java 8 / 21 / 25** (compilato con `<release>8</release>`, compatibile con runtime moderni) |
| Stile EJB | **EJB 3.2** — `SessionBean`, `EJBLocalHome`, `EJBLocalObject` (pienamente supportati su Liberty) |
...
| Web Service | **JAX-WS 2.2** / SOAP (con retrocompatibilità per client legacy) |
| Livello Web | **Servlet 3.1** + **JSP 2.3** |
| Tag di Vista | **JSTL 1.2** + **EL 3.0** (forniti nativamente da Liberty) |
| File statici | Serviti direttamente dal motore web di Liberty |
| Email | **JavaMail 1.5** tramite JNDI `mail/MercatoMail` (con supporto TLS / bypass SSL se richiesto) |
| Build | **Maven 3** (progetto EAR multi-modulo con Liberty Maven Plugin) |
...
| Strumento | Versione |
|---|---|
| Java JDK | **8 / 11 / 17 / 21 / 25** (compatibile con runtime moderni) |
| Maven | 3.6+ |
| PostgreSQL | 15+ |
| Open Liberty / WebSphere Liberty | **25.0+ / 26.0+** |
| Python | 3.x (solo per la generazione immagini SVG) |

> **Importante:** anche se si compila con un JDK più recente (es. JDK 21 o 25), il POM è configurato con target di compilazione Java 8 per garantire la massima compatibilità e portabilità di esecuzione su tutti i runtime Java EE 7 compatibili.

---

## Build e Gestione con Open Liberty

Il modulo EAR include il plugin `liberty-maven-plugin` per automatizzare l'installazione delle feature, l'avvio e il deploy su Liberty.
...
## Open Liberty (Modernizzazione Cloud-Native)

PaskinoMercato è stato modernizzato con successo per essere eseguito su **Open Liberty** (o WebSphere Liberty), trasformandolo in un'applicazione cloud-native leggera e containerizzabile.

### 1. Migrazione EJB 2.x & JNDI
Durante la migrazione, sono state apportate modifiche mirate per garantire la piena compatibilità con il runtime Liberty:
* **Risoluzione Conflitto EJB (BUSINESS_LOCAL)**: In EJB 3.x+ e Liberty, se una classe bean implementa direttamente la propria interfaccia locale EJB 2.x (es. `CatalogoBean implements SessionBean, CatalogoLocal`) che estende `EJBLocalObject`, il container la interpreta erroneamente come una *Business Local Interface* e solleva un'eccezione `EJBConfigurationException`. Per risolvere il problema, le firme delle classi `CatalogoBean`, `ClienteBean` e `OrdineBean` sono state alleggerite rimuovendo l'implementazione diretta dell'interfaccia locale (mantenendo solo `implements SessionBean`). Liberty risolve comunque i binding e genera i proxy corretti tramite la dichiarazione in `ejb-jar.xml`.
* **Resource Reference JNDI**: È stata inserita la dichiarazione `<resource-ref>` esplicita per `jdbc/MercatoDB` all'interno di `CatalogoBean` nel descrittore `ejb-jar.xml` per mappare correttamente il contesto locale `java:comp/env/jdbc/MercatoDB` alla risorsa globale del server.

### 2. Configurazione di Liberty (`server.xml` & `server.env`)
La configurazione di Liberty è centralizzata in `paskinomercato-ear/src/main/liberty/config/server.xml`.
Le credenziali sensibili del database non sono incluse nel file `server.xml` per motivi di sicurezza, ma vengono lette dinamicamente tramite variabili d'ambiente configurate nel file `paskinomercato-ear/src/main/liberty/config/server.env`:

```env
MercatoDBAlias_user=mercato
MercatoDBAlias_password=changeme
```

Il driver JDBC di PostgreSQL (`postgresql-42.7.13.jar`) deve essere presente nel percorso configurato, ad esempio `/tmp/postgresql-42.7.13.jar`.

### 3. Comandi di Gestione con Maven Liberty Plugin

Il modulo EAR include il plugin `liberty-maven-plugin` per automatizzare l'installazione, la configurazione e il deploy di Liberty. Eseguire i seguenti comandi dalla cartella radice del progetto:
...
* **Installazione delle Feature Liberty (es. JSP, Servlet, EJB Lite, EJB Home, JDBC)**:
  ```bash
  mvn -pl paskinomercato-ear liberty:install-feature
  ```

* **Deploy dell'applicazione (EAR)**:
  ```bash
  mvn -pl paskinomercato-ear liberty:deploy
  ```

* **Avvio del server Liberty in background**:
  ```bash
  mvn -pl paskinomercato-ear liberty:start
  ```

* **Arresto del server Liberty**:
  ```bash
  mvn -pl paskinomercato-ear liberty:stop
  ```

* **Avvio in Modalità Sviluppo (Dev Mode - supporta Hot Reloading del codice e dei file JSP)**:
  ```bash
  mvn -pl paskinomercato-ear liberty:dev
  ```

### 4. Verifica di Funzionamento
Una volta avviato il server, l'applicazione risponderà ai seguenti indirizzi locali:
* **Home Page**: [http://localhost:9084/paskinomercato/](http://localhost:9084/paskinomercato/)
* **Catalogo Prodotti (connesso a Postgres)**: [http://localhost:9084/paskinomercato/catalogo](http://localhost:9084/paskinomercato/catalogo)

---

## Appendice: Configurazione WebSphere Legacy
...
## Appendice: Deploy su WebSphere Legacy (wsadmin)
...
| Endpoint SOAP | `http://localhost:9084/paskinomercato/MercatoService` |
| WSDL | `http://localhost:9084/paskinomercato/MercatoService?wsdl` |
